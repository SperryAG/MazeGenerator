...
//		complexity = calcComplexity();
	...
	private Map<Integer, Set<Pair>> optimalPathCoords(){//this should return the map of integer and set pairs of stuff. 
		//local variables used in optimalPathCoords: localVisited BFS search to find optimalPath availableCoords to iterate through
		...
		//beginning with the startNode and finding all neighbors at 1 step away, 2 step away, 3 step away etc.
		//Will add the starting node to level 0 steps away and its neighbors to 1 step away.
		...
		//while looping to check nodes in increasing steps away until you find the end node. 
	...
	//this will update and replace a Node to ensure that no changes are lost in node array. 
	...
	{
		//local variables, that we instantiate. setOptLocal always starts at end node.
		//optimalPathCoords is the nodes that are the optimal path from starting to end notated with howmany steps away from start they are
		//we start from the end node and loop backwards to step back towards the front.
		...
		for(int i = optimalPathCoords.size() -1; i >= 0; i--){//for loop backwards through all values in the map of optimalNodes
			...
			for(Pair pair : currentSet){//for all values in the current level of the map
				...
				//only Set true if the level below has the neighbor that's set to true. This ensures that we don't mark wrong nodes.
				else{
